...
class LoginApp:
    def __init__(self, root):
        """
        Khởi tạo giao diện đăng nhập, đăng ký và quên mật khẩu.
        Thiết lập cửa sổ, các thành phần giao diện và căn giữa màn hình.
        """
        self.root = root
        ...
        # Label "Quên mật khẩu?" căn phải, nhỏ, màu xanh, click gọi reset_password
        lbl_forgot = tk.Label(
            frame,
            text="Quên mật khẩu?",
            fg="#1976d2",
    ...
    def login(self):
        ...
        Nếu đúng, mở giao diện Admin hoặc User tương ứng.
    ...
    def register(self):
        ...
        Kiểm tra trùng tài khoản, thêm user mới nếu hợp lệ.
        """
        ...
        def do_register():
            """
            Xử lý đăng ký tài khoản mới khi nhấn nút Đăng ký.
            Kiểm tra nhập đủ thông tin, kiểm tra trùng tài khoản.
            """
            ...
            reg_win.destroy()
            self.root.deiconify()  # Hiện lại cửa sổ đăng nhập

        def on_close():
            """
            Xử lý khi đóng cửa sổ đăng ký: hiện lại cửa sổ đăng nhập.
            """
            self.root.deiconify()
            reg_win.destroy()
    ...
    def reset_password(self):
        ...
        - Ẩn cửa sổ đăng nhập.
        - Hiện hộp thoại nhập tài khoản, kiểm tra tồn tại.
        - Nếu đúng, cho nhập mật khẩu mới và cập nhật.
    ...
    def custom_askstring(self, title, prompt, icon_path, show=None):
        """
        Tạo hộp thoại nhập custom với icon, tiêu đề, kích thước, font, căn giữa màn hình.
        Args:
            title (str): Tiêu đề hộp thoại.
            prompt (str): Nội dung hướng dẫn.
            icon_path (str): Đường dẫn file icon.
            show (str): Nếu là '*', sẽ ẩn ký tự nhập (dùng cho mật khẩu).
        Returns:
            str hoặc None: Giá trị nhập được hoặc None nếu Cancel.
        """
        dialog = tk.Toplevel()
        dialog.title(title)
